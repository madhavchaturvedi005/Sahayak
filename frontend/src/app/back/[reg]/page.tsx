...
export default function BackPage() {
  ...
  const shareUrl = useMemo(() => {
    if (typeof window === 'undefined') return `/back/${encodeURIComponent(reg)}`
    return `${window.location.origin}/back/${encodeURIComponent(reg)}`
  }, [reg])
    ...
    <div className="page-wrap mx-auto max-w-[800px] space-y-6 pb-16">
      ...
      <GlassCard hover={false}>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link href={`/status/${encodeURIComponent(row.registration_id)}`} className="btn-secondary">
            View status
          </Link>
        </div>
      </GlassCard>
